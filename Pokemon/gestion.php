...
<body>
    ...
    // 3. INFO POKEMON (Corregido el nombre del botón)
    if (isset($_POST["informacion"], $_POST["pokemon"])) {
        $idPokemon = $_POST["pokemon"];
        if (isset($_SESSION["pokemons"][$idPokemon])) {
    ...
    // 4. MOSTRAR INFO ENTRENADORA (Corregido para que funcione tu botón)
    if (isset($_POST["mostrarInfo"], $_POST["entrenadora"])) {
        $idEntrenadora = $_POST["entrenadora"];
        if (isset($_SESSION["entrenadoras"][$idEntrenadora])) {
            // Llamamos a MostrarEquipo para ver el nombre y sus Pokémon asignados
            $mensaje = $_SESSION["entrenadoras"][$idEntrenadora]->MostrarEquipo();
        }
    }

    // 5. CAZAR (Corregido el nombre del botón)
    if (isset($_POST["cazar"], $_POST["entrenadora"], $_POST["pokemon"])) {
        ...
        if (isset($_SESSION["entrenadoras"][$idEntrenadora]) && isset($_SESSION["pokemons"][$idPokemon])) {
            $_SESSION["entrenadoras"][$idEntrenadora]->CazarPokemon($_SESSION["pokemons"][$idPokemon]);
            $mensaje = "Pokémon capturado correctamente.";
        }
    }
